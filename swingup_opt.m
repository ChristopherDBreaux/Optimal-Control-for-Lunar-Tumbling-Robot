function swingup_opt(input, folder_name, thread)
    % This function performs a YALMIP optimization for the swingup
    ...
    % Wheel velocity before brake
    formatted_solution.ww_brake = x4_sol(step_brake);
    % Body velocity after brake
    ...
    % Save to file
    file_name = folder_name + "/swingup_" + string(input.objective) + info + "N" + string(N) + "_s" + string(s) + "_xi" + strjoin(string(xi),"_") + "_thread" + string(thread) + ".mat";
